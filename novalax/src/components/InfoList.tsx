...
import { useAppKitState, useAppKitTheme, useAppKitAccount, useWalletInfo, useAppKitProvider, useAppKitNetworkCore } from "@reown/appkit/react";
		...
		const checkTransactionStatus = async () => {
			...
			if (isEthChain && hash) {
				try {
					...
					setStatusEthTx(receiptWithStatus?.status === 1 ? "Success" : receiptWithStatus?.status === 0 ? "Failed" : "Pending");
				} catch (err) {
		...
		<>
			...
			{hash && isEthChain && (
				<section>
					...
					<pre>
						Hash:{" "}
						<a href={`https://hashscan.io/${state.selectedNetworkId?.toString() == "eip155:296" ? "testnet/" : ""}transaction/${hash}`} target="_blank">
							{hash}
						</a>
						<br />
						Status: {statusEthTx}
			...
			{txId && !isEthChain && (
				<section>
					...
					<pre>
						Id:
						<a href={`https://hashscan.io/${state.selectedNetworkId?.toString() == "hedera:testnet" ? "testnet/" : ""}transaction/${txId}`} target="_blank">
							{txId}
						</a>
						<br />
					</pre>
