...
import { getMethodConfig, type FieldConfig } from "@/utils/methodConfigs";
import type { HederaProvider } from "@hashgraph/hedera-wallet-connect";
import type { ChainNamespace } from "@reown/appkit/networks";
import { useAppKitAccount, useAppKitNetworkCore, useAppKitProvider, useAppKitState, useDisconnect } from "@reown/appkit/react";
...
const ActionButtonList = ({ sendHash, sendTxId, sendSignMsg, sendNodeAddresses, ethTxHash, setLastFunctionResult, onDisconnect }: ActionButtonListProps) => {
	...
	const { executeHederaMethod } = useHederaMethods(walletProvider as HederaProvider, address!, sendTxId, sendSignMsg, sendNodeAddresses);
	...
	const createMethodButton = (methodName: string, requiresConnection = true) => (
		<button key={methodName} onClick={() => openModal(methodName)} disabled={(requiresConnection && !isConnected) || isLoading} className="method-button">
			{methodName}
		</button>
	);
	...
	return (
		<div>
			...
			<Web3Modal isOpen={isModalOpen} onClose={closeModal} onSubmit={handleModalSubmit} title={modalTitle} fields={modalFields} isLoading={isLoading} />
			<>
				{isConnected && (
					<>
						<div>
