...
import { BrowserProvider, formatEther, JsonRpcSigner, parseEther, getBigInt, hexlify, toQuantity, type BigNumberish, type TypedDataField } from "ethers";
...
import { jsonRpcProvider } from "@/helpers/config";
...
export const useEthereumMethods = ({ walletProvider, chainId, address, ethTxHash, sendHash, sendSignMsg }: UseEthereumMethodsProps) => {
	const [signedEthTx, setSignedEthTx] = useState<string>();

	const browserProvider = walletProvider && chainId ? new BrowserProvider(walletProvider, chainId) : undefined;
	const signer = walletProvider && address && browserProvider ? new JsonRpcSigner(browserProvider, address) : undefined;
	const rpcProvider =
		walletProvider && chainId
			...
			: {
					request: ({ method, params }: { method: string; params: unknown[] }) => jsonRpcProvider.send(method, params as never[]),
			  };
	...
	const execute = async (methodName: string, params: Record<string, string>) => {
		switch (methodName) {
			...
			case "eth_signTypedData": {
				...
				const signature = await signer.signTypedData(domain, eip712Types as unknown as Record<string, TypedDataField[]>, value);
				sendSignMsg(signature);
				return signature;
			}
