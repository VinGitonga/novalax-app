...
import { AccountInfo, AccountInfoQuery, Hbar, Transaction as HederaTransaction, TransactionId, TransferTransaction } from "@hashgraph/sdk";
import { HederaProvider, queryToBase64String, type SignAndExecuteQueryParams, type SignMessageParams, transactionToBase64String } from "@hashgraph/hedera-wallet-connect";
...
export const useHederaMethods = (walletProvider: HederaProvider, address: string, sendTxId: (id: string) => void, sendSignMsg: (msg: string) => void, sendNodeAddresses: (nodes: string[]) => void) => {
	...
	const execute = async (methodName: string, params: Record<string, string>) => {
		switch (methodName) {
			...
			case "hedera_executeTransaction": {
				if (!signedHederaTx) throw Error("Transaction not signed, use hedera_signTransaction first");
			...
			case "hedera_signAndExecuteTransaction": {
				...
				const accountId = address;
				const hbarAmount = new Hbar(Number(p.amount));
				const transaction = new TransferTransaction().setTransactionId(TransactionId.generate(accountId)).addHbarTransfer(accountId, hbarAmount.negated()).addHbarTransfer(p.recipientId, hbarAmount);
